...
async def export_graph(graph: GraphData):
    ...
    for nid in order:
        ...
        node_cls = registry.get(node_def["type"])
        params = node_def.get("data", {}).get("params", {})
        var_name = f"node_{nid.replace('-', '_')}"
        lines.append(f"# {node_def['type']} (id: {nid})")
        lines.append(f"{var_name}_params = {params!r}")
        lines.append(f"# Execute {node_def['type']} with params above")
        lines.append("")
